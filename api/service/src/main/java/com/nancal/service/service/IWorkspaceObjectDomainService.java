...
import com.nancal.common.utils.*;
import com.nancal.framework.common.converter.FrameworkConverterProperties;
...
public interface IWorkspaceObjectDomainService extends IBusinessObjectDomainService {
    ...
    Logger log = LoggerFactory.getLogger(IWorkspaceObjectDomainService.class);
    ...
    default WorkspaceObjectResp save(BusinessObjectReq req) {
        ...
        if (StrUtil.isBlank(req.getObjectType())){
        ...
        // 保存当前对象
        SpringUtil.getBean(EntityManager.class).persist(entity);
        // 构建响应对象的全包名
    ...
    default BusinessObjectResp update(BusinessObjectReq req) {
        ...
        WorkspaceObjectEntity entity = EntityUtil.getById(objectType,myReq.getUid());
        ...
        // 检查数据是否发生过修改
        if (req.equals(entity)) {
            BeanUtil.copyPropertiesIgnoreNull(entity, instance);
     ...
     * @param args
     */
    default String getVerifyAuthority(WorkspaceObjectEntity entity, OperatorEnum operator, String tipType,String... args) {
       return getVerifyAuthority(entity,operator,DataTypeEnum.CURRENT_DATA,tipType,args);
    }

    default String getVerifyAuthority(WorkspaceObjectEntity entity, OperatorEnum operator, DataTypeEnum dataType,String tipType,Object... args) {
        try {
           ...
           return MessageSourceUtil.getMessage(tipType,args);
        }
    }
...
//                    .registerWriteHandler(new ExcelUtils(map))
    ...
    default void download(String filePath, HttpServletResponse response) {
        if (StrUtil.isEmpty(filePath)) {
            throw new ServiceException(ErrorCode.ERROR, "文件路径不存在");
        }
        JPAQueryFactory jpaQueryFactory = SpringUtil.getBean(JPAQueryFactory.class);
        QFileStorageEntity fileStorage = QFileStorageEntity.fileStorageEntity;
        BooleanBuilder builder = new BooleanBuilder();
        builder.and(fileStorage.filePath.eq(filePath));
        FileStorageEntity storageEntity = jpaQueryFactory.selectFrom(fileStorage).where(builder).fetchFirst();
        if (Objects.nonNull(storageEntity)) {
            // 校验权限
            this.verifyAuthority(storageEntity,OperatorEnum.DownLoad);
        }
        ...
        try {
            ...
            String fileName = cn.hutool.core.net.URLEncoder.createDefault().encode(FileUtil.getName(filePath), StandardCharsets.UTF_8);
            URL url = new URL(cn.hutool.core.net.URLEncoder.createDefault().encode(filePath, StandardCharsets.UTF_8));
            HttpURLConnection conn = (HttpURLConnection)url.openConnection();
            conn.setConnectTimeout(5*1000);
            //防⽌屏蔽程序抓取⽽返回403错误
            conn.setRequestProperty(HttpHeaders.USER_AGENT, "Mozilla/4.0 (compatible; MSIE 5.0; Windows NT; DigExt)");
            ...
            try {
                ...
                throw new ServiceException(ErrorCode.ERROR, "文件下载错误");
            }finally {
                if(output!=null){
                    output.close();
                }
     ...
     * 上传文件
     *
     * @param filePath 文件全路径（包括文件名称后缀）
     * @author 徐鹏军
     * @date 2022/5/26 08:26
     */
    default FileAttrReq redepositFile(String filePath) {
        return redepositFile(null, filePath);
    }
     ...
     * @date 2022/5/26 08:26
     */
    default FileAttrReq redepositFile(InputStream in, String filePath) {
        // 先将流中的文件写入到本地磁盘临时文件中
        ...
        try {
            ...
            if (in != null) {
                FileUtil.writeFromStream(in, file, true);
            }
            FileItem fileItem = FileItemUtil.getMultipartFile(file,FileUtil.mainName(file));
            MultipartFile multipartFile = new CommonsMultipartFile(fileItem);
            LezaoResult<IntegrationFileVo> upload = SpringUtil.getBean(RemoteLezaoIntegrationService.class).upload(multipartFile, "lzdigit");
            ...
            fileAttrReq.setFilePath(upload.getData().getUrl());
            fileAttrReq.setFileSize(upload.getData().getSize());
            fileAttrReq.setFileType(upload.getData().getExt());
            fileAttrReq.setBucketName(upload.getData().getGroup());
            return fileAttrReq;
        } catch (Exception e) {
            log.error("文件转存失败", e);
    ...
    /**
     * 发送消息
     * @param msgCode 从MsgConstant常量中获取
     * @param userIdList 用户数组
     * @param param 普通对象，或Map均可
     */
    default void sendMsg(String msgCode,List<String> userIdList,Object param){
        List<Pair<String,Object>> pairList = new ArrayList<>();
        pairList.add(Pair.of(MsgTemplateEntity.MSG_CODE,msgCode));
        pairList.add(Pair.of(MsgTemplateEntity.DEL_FLAG,false));
        JPAQuery<MsgTemplateEntity> dynamicEqQuery = EntityUtil.getDynamicEqQuery(MsgTemplateEntity.class, pairList);
        MsgTemplateEntity msgTemplateEntity = dynamicEqQuery.fetchFirst();
        if(ObjectUtil.isNull(msgTemplateEntity)){
            log.error("模版不存在,msgCode={}",msgCode);
            throw new ServiceException(ErrorCode.E_10, "模版不存在");
        }
        RemoteItMsgSystemService itMsgSystemService = SpringUtil.getBean(RemoteItMsgSystemService.class);
        ItMessageTo itMessageTo = new ItMessageTo();
        itMessageTo.setTitle(ExpressUtil.parseObj(msgTemplateEntity.getTitle(),param));
        itMessageTo.setBody(ExpressUtil.parseObj(msgTemplateEntity.getBody(),param));
        itMessageTo.setMsgType(msgTemplateEntity.getMsgType()+"");
        itMessageTo.setUserIds(userIdList);
        itMessageTo.setMailType("1");
        itMessageTo.setChannelType("1");
        itMsgSystemService.messageAdd(itMessageTo);
    }
     ...
     * @return: {@link WorkspaceObjectResp}
     */
    default WorkspaceObjectResp previewView(IdRequest id){
        String objectType = new Gte4PartRevisionEntity().getObjectType();
        DictUtil dictUtil = SpringUtil.getBean(DictUtil.class);
        ItemRevision itemRevision = SpringUtil.getBean(ItemRevisionFactory.class).create();
        Item item = SpringUtil.getBean(ItemFactory.class).create();
        if (ItemEntity.class.isAssignableFrom(EntityUtil.getEntityClass(id.getObjectType()))){
            WorkspaceObjectEntity lastVersion = item.getLastVersion(id.getUid(), id.getObjectType());
            id.setUid(lastVersion.getUid());
            id.setObjectType(lastVersion.getObjectType());
        }
        //查找关联的BOM
        RelationEntity entity = null;
        String dictType = "";
        String relation = "";
        String pattern = id.getObjectType();
        String postfix = StrUtil.UNDERLINE+"relevancy";
        String uid = id.getUid();
        do {
            List<WorkspaceObjectEntity> allSequences = itemRevision.getAllSequences(uid, pattern);
            List<String> stringList = allSequences.stream().map(WorkspaceObjectEntity::getUid).collect(Collectors.toList());
            dictType = pattern + postfix;
            Map<String, String> codeValueMap = dictUtil.getCodeValueMap(dictType);
            if (!codeValueMap.containsKey(pattern)){
                return null;
            }
            relation = codeValueMap.get(pattern);
            ...
            entity = (RelationEntity)entities;
            pattern = entity.getRightObjectType();
            uid = entity.getRightObject();
        }while (!entity.getRightObjectType().equals(objectType));
