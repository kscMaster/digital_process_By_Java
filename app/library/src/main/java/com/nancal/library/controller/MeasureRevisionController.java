...
@Slf4j
@RestController
@RequestMapping("/MeasureRevision")
@Api(tags = "量具版本管理")
public class MeasureRevisionController {

    @Qualifier("measureRevisionDomainServiceImpl")
    @Autowired
    private IMeasureRevisionDomainService service;
    ...
    public Response<BusinessObjectResp> update(@RequestBody @Validated(UpdateGroup.class) LibraryAttributeMeasureReq req) {
    ...
    @PreAuthorize
    @PostMapping("/upgrade")
    public Response<WorkspaceObjectResp> upgrade(@RequestBody @Validated(UpdateGroup.class) LibraryAttributeMeasureReq req) {
        return Response.of(service.customUpgrade(req));
    }

    @PostMapping("/like")
    @ApiOperation(value = "模糊查询")
    public TableResponse<List<Map<String,Object>>> like(@RequestBody @Validated(LikeGroup.class) TableRequest<MeasureRevisionReq> req) {
