...
@Service
public class MeasureDomainServiceImpl extends CustomFieldPoiWordAnalysis<LibraryAttributeMeasureReq> implements  IMeasureDomainService, IItemDomainServiceAdaptor{

    @Autowired
    private IExtraPropertyDataDomainService iExtraPropertyDataDomainService;

    @Autowired
    private MeasureDomainServiceImpl measureDomainService;
    ...
    public WorkspaceObjectResp save(BusinessObjectReq req) {
        LibraryAttributeMeasureReq auxiliaryMaterialReq = (LibraryAttributeMeasureReq)req;
        WorkspaceObjectResp save = IItemDomainServiceAdaptor.super.save(auxiliaryMaterialReq.getMeasureRevisionReq());
    ...
    public Response<Gte4ImportRevisionResp> importExcel(MultipartFile file, String leftObjec, String  leftObjectType, boolean isExcel){
        ...
        dictTypes.add(DictConstant.MEASURE_EXTRADATA);
        ...
        List<LibraryAttributeMeasureReq> analysis = super.analysis(file);
        if (!isExcel){
        ...
        List<List<LibraryAttributeMeasureReq>> split = CollUtil.split(analysis, 200);
        AtomicInteger atomicInteger = new AtomicInteger();
        split.forEach(l ->{
            CompletableFuture<Void> future = CompletableFuture.runAsync(() -> {
                l.forEach(revisionReq ->{
                    RequestContextHolder.setRequestAttributes(requestAttributes);
                    MeasureRevisionReq equipmentRevisionReq = revisionReq.getMeasureRevisionReq();
                    equipmentRevisionReq.setLeftObject(leftObjec);
                    equipmentRevisionReq.setLeftObjectType(leftObjectType);
                    WorkspaceObjectResp insert = measureDomainService.insert(revisionReq.getMeasureRevisionReq());
        ...
        measureDomainService.statistics(leftObjectType,leftObjec,atomicInteger.get());
        return Response.of();
    }
    ...
    public void exportExcelModel(HttpServletResponse request){
        ...
        ExportExcelUtil.customExportExcels(request,required,titles,"资源库模板-量具");
    }


    @Override
    public TableResponse<List<Map<String,Object>>> customPage(TableRequest<? extends ItemRevisionReq> req){
        TableResponse<WorkspaceObjectResp> response = IMeasureDomainService.super.pagePlus(req);
