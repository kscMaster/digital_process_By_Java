...
@Slf4j
@RestController
@RequestMapping("/AuxiliaryMaterialRevision")
@Api(tags = "辅料版本管理")
public class AuxiliaryMaterialRevisionController {

    @Qualifier("auxiliaryMaterialRevisionDomainServiceImpl")
    @Autowired
    private IAuxiliaryMaterialRevisionDomainService service;
    ...
    public Response<BusinessObjectResp> update(@RequestBody @Validated(UpdateGroup.class) LibraryAttributeAuxiliaryReq req) {
    ...
    @PreAuthorize
    @PostMapping("/upgrade")
    public Response<WorkspaceObjectResp> upgrade(@RequestBody @Validated(UpdateGroup.class) LibraryAttributeAuxiliaryReq req) {
        return Response.of(service.customUpgrade(req));
    }

    @PostMapping("/like")
    @ApiOperation(value = "模糊查询")
    public TableResponse<List<Map<String,Object>>> like(@RequestBody @Validated(LikeGroup.class) TableRequest<AuxiliaryMaterialRevisionReq> req) {
