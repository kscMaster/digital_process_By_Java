...
@Api(tags = "BOM节点管理")
public class MBOMNodeController {
    ...
    public Response<List<BOMNodeResp>> findOne(@RequestBody @Validated FindReq id) {
        BOPNodeViewResp viewResp = service.findOne(BOPNodeViewResp.class, id, AppNameEnum.MBOM);
    ...
    public Response<List<BOMNodeResp>> find(@RequestBody @Validated FindReq req) {
        req.setDeep(-1);
        BOMNodeResp viewResp = service.find(BOPNodeViewResp.class, req, AppNameEnum.MBOM);
        return Response.of(ObjectUtil.isNull(viewResp)?null:Collections.singletonList(viewResp));
    }
    ...
    public Response<List<BOMNodeResp>> findRoot(@RequestBody @Validated FindReq id) {
        BOPNodeViewResp viewResp = service.find(BOPNodeViewResp.class, id, AppNameEnum.MBOM);
        return Response.of(ObjectUtil.isNull(viewResp)?null:Collections.singletonList(viewResp));
    }
    ...
    public Response<List<BOMNodeResp>> findChildList(@RequestBody @Validated FindReq id) {
        BOPNodeViewResp root = service.find(BOPNodeViewResp.class, id, AppNameEnum.MBOM);
    ...
    public Response<List<Object>> saveBomNode(@RequestBody @Validated(BOMNodeReq.Paste.class) ValidList<BOMNodeReq> req) {
        return Response.of(service.parseData(req,AppNameEnum.MBOM, DictConstant.GTE4PROCESS_BOMSTRUCTURE));
    }
    ...
    public Response<CompareResp> compare(@RequestBody @Validated CompareReq req) {
        return Response.of(service.compare(req,AppNameEnum.MBOM));
    }
