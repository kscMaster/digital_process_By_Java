...
import com.nancal.api.utils.excel.verify.EBomVerify;
...
@Service
public class Gte4PartDomainServiceImpl extends ExcelPoiWordAnalysis<Gte4PartRevisionImportReq> implements IGte4PartDomainService {

    @Autowired
    private IBOMNodeDomainService ibomNodeDomainService;

    @Autowired
    private Gte4PartDomainServiceImpl gte4PartDomainService;
    ...
    @Autowired
    private IGte4PartRevisionDomainService iGte4PartRevisionDomainService;
    ...
    public WorkspaceObjectResp saveBomReq(BusinessObjectReq req) {
        ...
        BOMNodeReq nodeReq = partBomReq.getBomNodeReq();
//        1、单独创建设计零件对象
        Gte4PartRevisionReq partRevisionReq = partBomReq.getPartRevisionReq();
        //根节点新建默认关联板子key
        partRevisionReq.setBoardKey("PB-000923");
        WorkspaceObjectResp save = IGte4PartDomainService.super.save(partRevisionReq);
//        2、在父级底下添加设计零件对象
        if (!Objects.isNull(nodeReq)) {
            //判断是否可以添加，取决于父级的是否具有编辑权限
            gte4PartDomainService.checkParentRevEidt(nodeReq);
            nodeReq.setChildItem(save.getUid());
            nodeReq.setChildItemType(new Gte4PartEntity().getObjectType());
            ibomNodeDomainService.createNode(nodeReq, AppNameEnum.EBOM);
        }
        return save;
    }
    @Override
    ...
    public Response<Gte4ImportRevisionResp> importExcel(MultipartFile file, Boolean isImport) {
        ...
        List<Gte4PartRevisionImportReq> analysis = excelImportVerify(file,statisticsMap);
        ...
        if (isImport) {
            ...
            if ((Integer)statisticsMap.get("failNum") == 0){
                ...
                verifyRank(analysis,statisticsMap,msgMap);
     ...
     * @return
     */
    public List<Gte4PartRevisionImportReq> excelImportVerify(MultipartFile file,Map<String,Object> statisticsMap){
        statisticsMap.put("okNum",0);
        statisticsMap.put("failNum",0);
        statisticsMap.put("errorMsg","");
        EBomVerify baseLibraryVerify = new EBomVerify();
    ...
    public WorkspaceObjectResp comparison(Gte4PartRevisionImportReq importExcelData,WorkspaceObjectResp parentObject){
        ...
        BOPNodeViewResp bopNodeViewResp = service.find(BOPNodeViewResp.class, findReq, AppNameEnum.EBOM);
        Item item = SpringUtil.getBean(ItemFactory.class).create();
        /**
         *  此方法出现的情况：
    ...
    public WorkspaceObjectResp topInsert(Gte4PartRevisionImportReq tree){
        //1、单独创建设计零件对象
        WorkspaceObjectResp save = gte4PartDomainService.save(tree);
        if (null != tree.getGte4PartRevisionImportReqList() && !tree.getGte4PartRevisionImportReqList().isEmpty()) {
            ...
            List<List<Gte4PartRevisionImportReq>> split = CollUtil.split(tree.getGte4PartRevisionImportReqList(), 1);
            split.forEach(l -> {
                CompletableFuture<Void> future = CompletableFuture.runAsync(() -> {
                    RequestContextHolder.setRequestAttributes(requestAttributes);
                    executeMethod(l, save);
                }, executor);
                all.add(future);
            });
        ...
        iGte4PartRevisionDomainService.associatedBoards(associatedBoardsReq);
        return save;
    }
    ...
    public WorkspaceObjectResp topUpdate(Gte4PartRevisionImportReq tree){
        ...
        BomEditRevisionResp gte4PartRevision = (BomEditRevisionResp)iGte4PartRevisionDomainService.update(gte4PartBomReq);
        WorkspaceObjectResp workspaceObjectResp = new WorkspaceObjectResp();
    ...
    public void recursionRankInsert(Gte4PartRevisionImportReq gte4PartRevisionImportReq
        ...
        //        1、单独创建设计零件对象
        WorkspaceObjectResp workspaceObjectResp = gte4PartDomainService.save(gte4PartRevisionImportReq);
  ...
  // 验证方法---------------------------------------------------------------------------------------------------------------------
     ...
     *  顶层是否已发布
     */
    public void verifyRank(List<Gte4PartRevisionImportReq> originalList,Map<String,Object> statisticsMap, Map<String,String> msgMap){
        List<Gte4PartRevisionImportReq> treeList = new ArrayList<>();
        for (int i = 1; i < 10000; i++) {
            String current = String.valueOf(i);
            List<Gte4PartRevisionImportReq> currentList = originalList.stream().filter(it -> current.equals(it.getRank())).collect(Collectors.toList());
            if (currentList.isEmpty()) {
                ...
                if (null != currentList.get(0) && null != currentList.get(0).getLifeCycleState() && !LifeCycleStateEnum.Working.name().equals(currentList.get(0).getLifeCycleState())){
            ...
            recursionVerify(this.getGte4PartRevisionImportReq(currentList.get(0)), statisticsMap, msgMap);
            ...
            for (int j = 1; j < treeList.size(); j++){
                Gte4PartRevisionImportReq original = treeList.get(j);
        ...
        for (int i = 0; i < originalList.size(); i++){
            Gte4PartRevisionImportReq original = originalList.get(i);
     ...
     * @param msgMap
     */
    public void recursionVerify(Gte4PartRevisionImportReq parentObject,Map<String,Object> statisticsMap, Map<String,String> msgMap){
        if (null != parentObject && null != parentObject.getGte4PartRevisionImportReqList()){
            parentObject.getGte4PartRevisionImportReqList().forEach(currentObject ->{
                // 验证更新异常信息
                if (StringUtils.isNotBlank(currentObject.getItemId())){
                    Gte4PartRevisionEntity objectEntity = this.getRevision(currentObject);
                    ...
                    if(StringUtils.isNotBlank(parentObject.getUid())) {
                        BOMNodeReq bomNodeReq = new BOMNodeReq();
                        ...
                        bomNodeReq.setChildItemType(new Gte4PartEntity().getObjectType());
                        try {
                            ibomNodeDomainService.checkNode(bomNodeReq, AppNameEnum.EBOM);
                        } catch (ServiceException e) {
                            addErrorMsg(currentObject.getRank(),"结构错误" +e.getMessage(), statisticsMap, msgMap);
                        }
                    }
                }
                recursionVerify(this.getGte4PartRevisionImportReq(currentObject), statisticsMap, msgMap);
            });
        }
    }
